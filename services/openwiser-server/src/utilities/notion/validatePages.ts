...
import { notionClient, prisma } from "..";

const NOTION_BOOKS_DATABASE_ID = process.env["NOTION_BOOKS_DATABASE_ID"]!;

export const validatePages = async () => {
  console.log("Validating Notion Pages");

  await checkMissingNotionPages();

  await checkNoNotionPage();
...
const checkMissingNotionPages = async () => {
  console.log("Checking for Missing Notion Pages");
